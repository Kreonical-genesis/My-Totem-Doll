...
public class ForgingScreenMixin {
	...
	//? if >=1.21.6 {
	@WrapOperation(at = @At(value = "INVOKE", target = "Lnet/minecraft/client/gui/DrawContext;drawTexture(Lcom/mojang/blaze3d/pipeline/RenderPipeline;Lnet/minecraft/util/Identifier;IIFFIIII)V"), method = "drawBackground")
	private void drawBackground(DrawContext instance, com.mojang.blaze3d.pipeline.RenderPipeline renderPipeline, Identifier identifier, int x, int y, float u, float v, int width, int height, int textureWidth, int textureHeight, Operation<Void> original) {
		if (this instanceof MTDAnvilScreen && MyTotemDollClient.getConfig().isModEnabled()) {
			original.call(instance, renderPipeline, identifier, x, y, u, v, 176, height, textureWidth, textureHeight);
			return;
		}
		original.call(instance, renderPipeline, identifier, x, y, u, v, width, height, textureWidth, textureHeight);
	}
	//?} elif >=1.21.2 {
